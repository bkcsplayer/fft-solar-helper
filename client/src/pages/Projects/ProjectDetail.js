...
const ProjectDetail = () => {
  ...
  const fetchLogs = async () => {
    try {
      // Logs are included in project detail now, but can be refreshed separately if needed
      // Currently project detail include logs, so we update logs from project data or separate endpoint
      // Let's use the data from project first, but a separate refresh is good for "Add Log"
      const response = await api.get(`/projects/${id}`);
      setLogs(response.data.logs || []);
    } catch (error) {
  ...
  const handleAddLog = async () => {
    ...
    try {
      await api.post(`/projects/${id}/logs`, { content: newLog, log_type: 'note' });
      setNewLog('');
      fetchProjectDetail(); // Refresh all including logs
    } catch (error) {
  ...
  const handleOpenAssignDialog = (staffId = '') => {
    if (!staffList.length) fetchStaffList();
    setAssignDialogOpen(true);
    setSelectedStaff(staffId);

    // Auto-select role if staff has one
    if (staffId) {
              ...
              <Box sx={{ display: 'flex', gap: 1, flexWrap: 'wrap' }}>
                {staffList.filter(s => s.is_active).map((staff) => {
                  ...
                  return (
                    <Chip
                      key={staff.id}
                      label={`${staff.name} (${staff.role === 'leader' ? '领队' : staff.role === 'electrician' ? '电工' : staff.role === 'installer' ? '安装' : '员工'})`}
                      avatar={<Box sx={{
                        bgcolor: hubColor,
                        color: 'white',
                        width: 24,
                      ...
                      onClick={() => handleOpenAssignDialog(staff.id)}
                      sx={{
                        bgcolor: bgColor,
                ...
                {staffList.length === 0 && <Typography variant="caption" color="text.secondary">未加载人员列表</Typography>}
              </Box>
            </Box>
          ...
          <Grid item xs={12} md={6}>
            <Card sx={modernCardStyle}>
              <CardContent sx={{ p: 3 }}>
                ...
                <Table size="small">
                  <TableBody>
                    <TableRow sx={{ '& td': { borderBottom: '1px solid #e2e8f0', py: 1.5, bgcolor: '#f8fafc' } }}>
                      <TableCell colSpan={2} sx={{ fontWeight: 600, color: '#10b981' }}>项目工程款</TableCell>
                    </TableRow>
                    <TableRow sx={{ '& td': { borderBottom: '1px solid #e2e8f0', py: 1.5 } }}>
                      <TableCell sx={{ color: '#64748b', pl: 4 }}>总瓦数 ({project.total_watt?.toLocaleString()} W) × 单价 (${project.client?.rate_per_watt}/W)</TableCell>
                      <TableCell align="right" sx={{ color: '#1e293b', fontWeight: 500 }}>
                        ${parseFloat(project.project_revenue || 0).toLocaleString()}
                      </TableCell>
                    </TableRow>

                    {/* Other Income */}
                    {project.finance_records?.filter(r => r.record_type === 'income').length > 0 && (
                      <>
                        <TableRow sx={{ '& td': { borderBottom: '1px solid #e2e8f0', py: 1.5, bgcolor: '#f8fafc' } }}>
                          <TableCell colSpan={2} sx={{ fontWeight: 600, color: '#10b981' }}>其他收入</TableCell>
                        </TableRow>
                        {project.finance_records.filter(r => r.record_type === 'income').map(record => (
                          <TableRow key={record.id} sx={{ '& td': { borderBottom: '1px solid #e2e8f0', py: 1.5 } }}>
                            <TableCell sx={{ color: '#64748b', pl: 4 }}>
                              {record.category}
                              {record.notes && <Typography component="span" variant="caption" sx={{ color: '#94a3b8', ml: 1 }}>({record.notes})</Typography>}
                              <Typography component="div" variant="caption" sx={{ color: '#cbd5e1' }}>{new Date(record.record_date).toLocaleDateString()}</Typography>
                            </TableCell>
                            <TableCell align="right" sx={{ color: '#1e293b', fontWeight: 500 }}>
                    ...
                    <TableRow sx={{ '& td': { borderBottom: 'none', py: 1.5, borderTop: '2px solid #e2e8f0' } }}>
                      <TableCell sx={{ fontWeight: 700, color: '#1e293b' }}>总收入</TableCell>
                      <TableCell align="right">
                        <Typography sx={{
                          fontWeight: 700,
                          color: '#059669',
                          ...
                          ${(
                            parseFloat(project.project_revenue || 0) +
                            (project.finance_records?.filter(r => r.record_type === 'income').reduce((sum, r) => sum + parseFloat(r.amount), 0) || 0)
                          ).toLocaleString()}
                        </Typography>
                      </TableCell>
                    </TableRow>
            ...
            <Card sx={modernCardStyle}>
              <CardContent sx={{ p: 3 }}>
                ...
                <Table size="small">
                  <TableBody>
                    <TableRow sx={{ '& td': { borderBottom: '1px solid #e2e8f0', py: 1.5, bgcolor: '#f8fafc' } }}>
                      <TableCell colSpan={2} sx={{ fontWeight: 600, color: '#f43f5e' }}>人工成本</TableCell>
                    </TableRow>
                    ...
                    {/* Other Expenses */}
                    {project.finance_records?.filter(r => r.record_type === 'expense').length > 0 && (
                      <>
                        <TableRow sx={{ '& td': { borderBottom: '1px solid #e2e8f0', py: 1.5, bgcolor: '#f8fafc' } }}>
                          <TableCell colSpan={2} sx={{ fontWeight: 600, color: '#f43f5e' }}>其他支出</TableCell>
                        </TableRow>
                        {project.finance_records.filter(r => r.record_type === 'expense').map(record => (
                          <TableRow key={record.id} sx={{ '& td': { borderBottom: '1px solid #e2e8f0', py: 1.5 } }}>
                            <TableCell sx={{ color: '#64748b', pl: 4 }}>
                              {record.category}
                              {record.notes && <Typography component="span" variant="caption" sx={{ color: '#94a3b8', ml: 1 }}>({record.notes})</Typography>}
                    ...
                    <TableRow sx={{ '& td': { borderBottom: 'none', py: 1.5, borderTop: '2px solid #e2e8f0' } }}>
                      ...
                      <TableCell align="right">
                        <Typography sx={{
                          ...
                          ${(
                            parseFloat(project.project_expense || 0) +
                            (project.finance_records?.filter(r => r.record_type === 'expense').reduce((sum, r) => sum + parseFloat(r.amount), 0) || 0)
                          ).toLocaleString()}
                        </Typography>
                      </TableCell>
                    </TableRow>
        ...
        <DialogContent sx={{ pt: 3 }}>
          <Grid container spacing={3}>
            ...
            {project.project_type === 'insurance' && (
              <Grid item xs={12}>
                <FormControl fullWidth sx={modernInputStyle}>
                  <InputLabel>工作阶段</InputLabel>
                  <Select
                    value={selectedPhase}
                    label="工作阶段"
                    onChange={(e) => setSelectedPhase(e.target.value)}
                  >
                    <MenuItem value="removal">拆除阶段 (Removal)</MenuItem>
                    <MenuItem value="installation">安装阶段 (Installation)</MenuItem>
                  </Select>
                </FormControl>
              </Grid>
            )}
        ...
        <DialogContent sx={{ pt: 3 }}>
          {!editingInverter && (
            <Box sx={{ mb: 3, p: 2, bgcolor: '#f8fafc', borderRadius: 2 }}>
              ...
              <Box sx={{ display: 'flex', gap: 1, flexWrap: 'wrap' }}>
                {[
                  { label: 'HMS-1600', brand: 'Hoymiles', model: 'HMS-1600', watt: 1600, type: 'micro' },
                  { label: 'HMS-800', brand: 'Hoymiles', model: 'HMS-800', watt: 800, type: 'micro' },
                  { label: 'HMS-400', brand: 'Hoymiles', model: 'HMS-400', watt: 400, type: 'micro' },
                  { label: 'APS-DS3L', brand: 'APsystems', model: 'DS3L', watt: 730, type: 'micro' },
                ].map((preset) => (
                  <Chip
                    key={preset.label}
                    label={preset.label}
                    size="small"
                    onClick={() => setInverterForm({
                      ...inverterForm,
                      brand: preset.brand,
                      model: preset.model,
                      watt_per_unit: preset.watt,
                      inverter_type: preset.type
                    })}
                    sx={{ cursor: 'pointer', '&:hover': { bgcolor: '#e2e8f0' } }}
                  />
                ))}
              </Box>
            </Box>
